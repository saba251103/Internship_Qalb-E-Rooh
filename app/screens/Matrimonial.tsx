/**
 * MatrimonialScreen.tsx — Qalb-E-Rooh
...
type BadgeType = 'recommended' | 'popular' | 'new' | null;
type AccordionItem = {
...
const DATA: AccordionItem[] = [
  {
    id: 'muzz', name: 'Muzz', tagline: "The World's Largest Muslim Marriage App",
    meta: 'iOS & Android · 10M+ Users', metaIcon: 'heart-outline',
    bio: "Muzz is the world's leading Muslim marriage app, connecting millions of single Muslims seeking halal relationships. It features built-in photo privacy, a Wali (guardian) mode so family can be involved, selfie verification for authenticity, and a robust filtering system by sect, ethnicity, and religiosity.",
    tags: [{ icon: 'shield-check-outline', label: 'Selfie Verified' }, { icon: 'account-group-outline', label: 'Wali Mode' }, { icon: 'lock-outline', label: 'Photo Privacy' }, { icon: 'star-outline', label: 'Free & Premium' }],
    ctaLabel: 'Open Website', website: 'https://muzz.com', badge: 'recommended',
  },
  {
    id: 'salams', name: 'Salams', tagline: 'Muslim Marriage & Friendship Platform',
    meta: 'iOS & Android · 4M+ Users', metaIcon: 'handshake-outline',
    bio: 'Formerly known as Minder, Salams brings together over 4 million Muslims worldwide. The app features an intuitive matching interface, strong anti-ghosting policies, incognito mode for private browsing, and curated profile review features.',
    tags: [{ icon: 'ghost-off-outline', label: 'Anti-Ghosting' }, { icon: 'check-decagram-outline', label: 'Profile Reviews' }, { icon: 'incognito', label: 'Incognito Mode' }, { icon: 'earth', label: 'Global Reach' }],
    ctaLabel: 'Open Website', website: 'https://www.salams.app', badge: 'popular',
  },
  {
    id: 'half-our-deen', name: 'Half Our Deen', tagline: 'Intentional & Private Matchmaking',
    meta: 'Web & Mobile · Curated Matches', metaIcon: 'ring',
    bio: 'Half Our Deen takes a thoughtful alternative to swiping culture. All profiles are private by default, and members only connect through curated matches. An in-depth personality compatibility questionnaire ensures alignment.',
    tags: [{ icon: 'gesture-swipe-left', label: 'No Swiping' }, { icon: 'lock-outline', label: 'Private Profiles' }, { icon: 'brain', label: 'Compatibility Test' }, { icon: 'web', label: 'Web Based' }],
    ctaLabel: 'Open Website', website: 'https://www.halfourdeen.com', badge: 'new',
  },
  {
    id: 'nikah-forever', name: 'NikahForever', tagline: 'Premium Muslim Matrimonial Service',
    meta: 'Web & App · Verified Profiles', metaIcon: 'mosque',
    bio: 'NikahForever is a premium matrimonial platform with a strong emphasis on verified, serious profiles. Dedicated matchmakers assist members, with options for family involvement throughout the process.',
    tags: [{ icon: 'account-check-outline', label: 'Verified Profiles' }, { icon: 'account-tie-outline', label: 'Matchmakers' }, { icon: 'heart-multiple-outline', label: 'Family Involved' }, { icon: 'crown-outline', label: 'Premium Service' }],
    ctaLabel: 'Open Website', website: 'https://www.nikahforever.com', badge: null,
  },
  {
    id: 'islamic-marriage', name: 'IslamicMarriage.com', tagline: 'Free Matrimonial for Muslims',
    meta: 'Web Based · No Subscription', metaIcon: 'star-crescent',
    bio: 'One of the oldest and most established Muslim matrimonial websites. Offers a completely free platform popular in South Asia, the UK, and North America, with a large database of profiles across all sects and ethnicities.',
    tags: [{ icon: 'currency-usd-off', label: '100% Free' }, { icon: 'account-multiple-outline', label: 'Large Database' }, { icon: 'map-marker-multiple', label: 'Worldwide' }, { icon: 'history', label: 'Established' }],
    ctaLabel: 'Open Website', website: 'https://www.islamicmarriage.com', badge: null,
  },
];

const BADGE_CFG = {
  recommended: { label: '⭐ Recommended', bg: C.teal,   text: '#FFF' },
  popular:     { label: '🔥 Most Popular', bg: C.gold,   text: '#FFF' },
  new:         { label: '🆕 New',          bg: C.isNew,  text: '#FFF' },
};
...
const AccordionCard = React.memo(({ item, views, isLoaded }: { item: AccordionItem; views: number; isLoaded: boolean }) => {
  ...
  const toggle = useCallback(() => {
    const next = !expanded;
    if (next) { setLocalViews(p => p + 1); MarketService.trackView(item.id); }
  ...
  const handleOpenWebsite = useCallback(async () => {
    Alert.alert('Open External Website', `You're about to open ${item.name} in your browser.`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Open', onPress: async () => {
        try {
          if (!hasClickedRef.current) { hasClickedRef.current = true; MarketService.trackClick(item.id); }
...
export default function MatrimonialScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const all = await MarketService.getBatchStats(DATA.map(a => a.id));
        if (isMounted) {
  ...
  return (
    <SafeAreaView style={scr.safe} edges={['top', 'left', 'right', 'bottom']}>
      <HeroHeader
        title="Matrimonial"
        arabicVerse="وَمِنْ آيَاتِهِ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَاجًا"
        verseTranslation="AND AMONG HIS SIGNS IS THAT HE CREATED FOR YOU SPOUSES"
        verseRef="AR-RUM 30:21"
        subtitle="Trusted platforms to complete half your deen"
        onBack={() => router.back()}
      />
      <View style={[scr.contentWrap, isTablet && scr.contentTablet]}>
        <FlatList
          data={DATA} keyExtractor={keyExtractor} renderItem={renderItem}
          ListHeaderComponent={<><OnboardingHint /><SectionLabel text="PLATFORMS" /><View style={{ height: vs(8) }} /></>}
          ListFooterComponent={
            <>
              <View style={scr.noteBanner}>
                <MaterialCommunityIcons name="information-outline" size={s(18)} color={C.teal} />
                <Text style={scr.noteTxt}>All listed platforms are Muslim-owned or Muslim-focused services dedicated to halal matrimonial connections.</Text>
              </View>
