/**
 * HalalTravelScreen.tsx — Qalb-E-Rooh
...
import { HalalTravelService } from '../services/halaltravelService';
...
type BadgeType = 'recommended' | 'popular' | 'essential' | null;
type AccordionItem = {
...
const DATA: AccordionItem[] = [
  {
    id: 'halal-travel-guide', name: 'Halal Travel Guide',
    tagline: 'Curated Halal Experiences & Tours', meta: 'London HQ · Global Group Tours', metaIcon: 'map-search',
    bio: 'Halal Travel Guide provides authentic, ethically-sourced travel experiences. They specialize in group tours and curated itineraries that ensure halal food, prayer spaces, and culturally respectful interactions are at the core of every trip.',
    tags: [{ icon: 'account-group', label: 'Ethical Tours' }, { icon: 'food-halal', label: 'Halal Meals' }, { icon: 'human-female', label: 'Solo Female Safe' }],
    ctaLabel: 'Open Website', website: 'https://halaltravelguide.net/', badge: 'recommended',
  },
  {
    id: 'halalbooking', name: 'HalalBooking',
    tagline: 'Global Halal-Friendly Stays', meta: 'Worldwide · 400K+ Properties', metaIcon: 'bed-outline',
    bio: 'The leading search engine for halal-conscious travelers. It allows you to filter hotels by "halal food on-site," "no-alcohol policy," and "ladies-only wellness facilities," ensuring a worry-free vacation.',
    tags: [{ icon: 'pool', label: 'Private Pools' }, { icon: 'glass-cocktail-off', label: 'Alcohol Free' }, { icon: 'check-decagram', label: 'Verified' }],
    ctaLabel: 'Open Website', website: 'https://halalbooking.com', badge: 'popular',
  },
  {
    id: 'hhwt', name: 'Have Halal, Will Travel',
    tagline: 'The Ultimate Muslim Travel Hub', meta: 'Singapore HQ · Global City Guides', metaIcon: 'compass-outline',
    bio: 'A massive resource for Muslim travelers, HHWT provides detailed food and travel guides for major cities worldwide, helping you find the best halal ramen in Tokyo or prayer rooms in London.',
    tags: [{ icon: 'notebook-outline', label: 'City Guides' }, { icon: 'silverware-fork-knife', label: 'Food Reviews' }, { icon: 'map-marker-radius', label: 'Local Tips' }],
    ctaLabel: 'Open Website', website: 'https://www.havehalalwilltravel.com/', badge: 'essential',
  },
  {
    id: 'halaltrip', name: 'HalalTrip',
    tagline: 'Trip Planning & In-Flight Tools', meta: 'Mobile App · Essential Tools', metaIcon: 'airplane',
    bio: 'An all-in-one travel companion featuring an in-flight prayer time calculator, Qibla compass, and a comprehensive database of halal-friendly restaurants and mosques across the globe.',
    tags: [{ icon: 'clock-outline', label: 'In-Flight Prayer' }, { icon: 'crosshairs-gps', label: 'Qibla Finder' }, { icon: 'briefcase-check', label: 'Trip Planner' }],
    ctaLabel: 'Open Website', website: 'https://www.halaltrip.com/', badge: null,
  },
];
...
const BADGE_CFG = {
  ...
  essential:   { label: '⭐ Essential',    bg: '#3B82F6', text: '#FFF' },
};
...
const AccordionCard = React.memo(({ item, views, isLoaded }: { item: AccordionItem; views: number; isLoaded: boolean }) => {
  ...
  const toggle = useCallback(() => {
    const next = !expanded;
    if (next) { setLocalViews(p => p + 1); HalalTravelService.trackView(item.id); }
  ...
  const handleLink = useCallback(() => {
    HalalTravelService.trackClick(item.id);
  ...
  return (
    <View style={[ac.card, expanded && ac.cardActive]}>
        ...
        <View style={ac.row}>
          ...
          <View style={ac.infoCol}>
            ...
            <View style={ac.metaRow}>
              <Feather name="globe" size={s(10)} color={C.gold} />
      ...
      <Animated.View style={[ac.dropdown, dropdownStyle]}>
        ...
        <TouchableOpacity style={ac.cta} activeOpacity={0.85} onPress={handleLink}
...
export default function HalalTravelScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const all = await HalalTravelService.getBatchStats(DATA.map(a => a.id));
        if (isMounted) {
  ...
  return (
    <SafeAreaView style={scr.safe} edges={['top', 'left', 'right', 'bottom']}>
      <HeroHeader
        title="Halal Travel"
        arabicVerse="قُلْ سِيرُوا فِي الْأَرْضِ فَانظُرُوا"
        verseTranslation="SAY, TRAVEL THROUGH THE LAND AND OBSERVE"
        verseRef="AL-ANKABUT 29:20"
        subtitle="Explore the world while preserving your values"
        onBack={() => router.back()}
      />
      <View style={[scr.contentWrap, isTablet && scr.contentTablet]}>
        <FlatList
          data={DATA} keyExtractor={keyExtractor} renderItem={renderItem}
          ListHeaderComponent={<><View style={{ height: vs(20) }} /><SectionLabel text="GLOBAL PLATFORMS" /></>}
          ListFooterComponent={<View style={{ height: Math.max(insets.bottom, vs(40)) + vs(40) }} />}
